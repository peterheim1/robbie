#!/usr/bin/env python
'''
This is the Master Control Program for Robbie The Robot
it will direct Robbies Actions and Behaviour
...
class Greeting():
    def __init__(self):
        ...
        #self.head.set_named_target("down")
        #self.head.go()
    ...
    def speech_callback(self,text):
        ...
        if matchObj:
            ...
            #test= self.Dictum(self.hold)
            #self.speak_text_service(test)
            self.task1 = re.sub('robbie ','',self.hold,1)
            self.hold1 = Dictum(self.task1)
            self.speak_text_service(str(self.hold1))
        else:
...
'''
    def speech_callback(self,text):
        ...
        if self.hold == "good morning":
            ...
            #self.head.set_named_target("ahead")
            #self.head.go()
            time.sleep(0.2)
