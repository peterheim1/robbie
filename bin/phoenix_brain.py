...
class CommonRobbieState(smach.State):
    ...
    def speech_callback(self, data):
        ...
        if self.state=="Default" and not face_detected:
                ...
                return
        if self.state=="Default" and sentence == "WHY DON'T YOU MOVE AROUND" and not robot_model.random_move:
                run_process("rosrun robbie qbo_random_move_face_recog.py")
            ...
            self.subscrib=rospy.Subscriber("/speech_text", String, self.self.speech_callback)
...
class questions(CommonRobbieState):
    def __init__(self):
...
class battery(CommonRobbieState):
    def __init__(self):
        smach.State.__init__(self, outcomes=['stop','auto_dock',''])
        self.state="battery"
    def execute(self, userdata):
        pass
        return self.next_state

class auto_dock(CommonRobbieState):
    def __init__(self):
        smach.State.__init__(self, outcomes=['stop',''])
        self.state="auto_dock"
    def execute(self, userdata):
        pass
        return self.next_state

class webi(CommonRobbieState):
    def __init__(self):
        smach.State.__init__(self, outcomes=['stop',''])
        self.state="Webi"
    ...
    def execute(self, userdata):
        ...
        #self.subscribe=rospy.Subscriber("/listen/"+lang+"_default", Listened, self.listen_callback)
        self.subscrib=rospy.Subscriber("/speech_text", String, self.speech_callback)
        speak_this("QBO WEB INTERFACE IS ACTIVE")
        ...
        if not rospy.is_shutdown():
            changeLang = rospy.ServiceProxy("/qbo_talk/festival_language", Text2Speach)
            changeLang("cmu_us_awb_arctic_clunits")
            speak_this("STOPPING QBO WEB INTERFACE")
            self.subscribe.unregister()
            rospy.loginfo("NextState: "+self.next_state)
...
class musicplayer(CommonRobbieState):
    def __init__(self):
...
'''
    ...
    object_active = False
    try:
       pass#object_active = rospy.get_param("/qbo_stereo_selector/move_head")
    except Exception:
       pass#print "No move head parameter yet"
...
def loadDictionary(lang_sym):
    ...
    #lang_path=en#roslib.packages.get_pkg_dir('qbo_brain')+"/config/lang/"
    #Load default dict
...
def main():
    ...
    #speak_text_service =rospy.Subscriber("/speech_text", String, speech_callback)
    #speak_text_service = rospy.ServiceProxy("/qbo_talk/festival_say_no_wait", Text2Speach)
    ...
    with sm:
        smach.StateMachine.add('default', default(), transitions={'mplayer':'music_player','phone':'phoneserver','webi':'webi','questions':'questions','battery':'battery','':'exit'})
        ...
        smach.StateMachine.add('battery', battery(), transitions={'stop':'default','auto_dock':'auto_dock', '':'exit'})
        smach.StateMachine.add('auto_dock', auto_dock(), transitions={'stop':'default', '':'exit'})
        

    sis= smach_ros.IntrospectionServer('server_name',sm,'/SM_ROOT')
    sis.start()

    speak_this("ROBBIE'S BRAIN IS ACTIVE")
