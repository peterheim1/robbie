...
black_board.task_list = OrderedDict([
    ('living_room', [Vacuum(room="lounge_room", timer=5)]),
    ('kitchen', [Mop(room="kitchen", timer=7)]),
    ('bathroom', [Scrub(room="dining_room", timer=9), Mop(room="dining_room", timer=5)])
    #('mats_room', [Vacuum(room="mats_room", timer=5)])
    ])
...
class CheckRoomCleaned(Task):
    def __init__(self,room):
        ...
        self.name = name
        self.room = room
        #print "rooom   " +  self.name
        print "rooom   " +  str(black_board.task_list[self.room])

    def run(self):
        try:
            #print "rooom   " +  black_board.task_list[self.room]
            if len(black_board.task_list[self.room]) != 0:
...
class CleanHouse():
    def __init__(self):
        ...
        with STAY_HEALTHY:
            ...
            CHARGE_ROBOT = ServiceTask("CHARGE_ROBOT", "battery_simulator/set_battery_level", SetBatteryLevel, 200, result_cb=self.recharge_cb)
